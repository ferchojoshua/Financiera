        ...
        <div class="col-md-12">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h4 class="mb-0 font-weight-bold"><i class="fas fa-users mr-2"></i>Administración de Usuarios</h4>
                    <div>
                        <a href="{{ route('admin.user.create') }}" class="btn btn-outline-light btn-sm">
                                ...
                                @if(isset($users) && $users->count() > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('admin.user.show', $user->id) }}" class="btn btn-sm btn-info text-white" title="Ver detalles">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('admin.user.edit', $user->id) }}" class="btn btn-sm btn-warning text-white" title="Editar">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @if(($user->role != 'superadmin' || auth()->user()->role == 'superadmin') && $user->id != auth()->id())
                                                <form action="{{ route('admin.user.destroy', $user->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Está seguro de que desea eliminar este usuario?')">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <tr>
                                        <td colspan="7" class="text-center py-4 text-muted">
                                            ...
                                            @if(request('search') || request('role') || request('status'))
                                                <a href="{{ route('admin.user.index') }}" class="btn btn-sm btn-primary mt-2">
